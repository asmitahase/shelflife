...
def calculate_renting_cost(issued_on, today ,renting_cost_per_day):
    number_of_days_delta = today - issued_on
    total_cost = number_of_days_delta.days*renting_cost_per_day
    return total_cost

def set_total_renting_cost_per_book(books):
    for book in books:
        book['total_renting_cost'] = calculate_renting_cost(book['issued_on'],datetime.today(),book['renting_cost'])
    return books
